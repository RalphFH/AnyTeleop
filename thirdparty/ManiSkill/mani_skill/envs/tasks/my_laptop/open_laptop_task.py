...
@register_env("OpenLaptop-v1", max_episode_steps=200)
...
class OpenLaptopEnv(BaseEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            # a little randomization for the laptop initial position
            base_pos = np.array([0.1, 0.05, 0.1])
            random_offset_x = np.random.uniform(-0.075, 0.05, size=2)
            random_offset_y = np.random.uniform(-0.35, 0.35, size=2)                           #set according to a lot of experiments.
