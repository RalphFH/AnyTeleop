...
class BaseEnv(gym.Env):
    ...
    def _setup_sensors(self, options: dict):
        ...
        if self.agent is not None:
            self._agent_sensor_configs = parse_camera_configs(self.agent._sensor_configs) # dict{uid: CameraConfig}
            self._sensor_configs.update(self._agent_sensor_configs) # dict{uid: CameraConfig}
        ...
        self._viewer_camera_config = self._viewer_camera_config["viewer"] # <class 'mani_skill.sensors.camera.CameraConfig'>


        # Now we instantiate the actual sensor objects
        self._sensors = dict() # dict{uid: Camera} | maniskill.sensors.camera.Camera

        articulation = self.agent.robot
        for uid, sensor_config in self._sensor_configs.items():
            # if uid in self._agent_sensor_configs:
            #     articulation = self.agent.robot # <class 'mani_skill.utils.structs.articulation.Articulation'>
            # else:
            #     articulation = None
        ...
        for uid, camera_config in self._human_render_camera_configs.items():
            self._human_render_cameras[uid] = Camera(
                camera_config,
                self.scene,
                articulation=articulation # modified by chenshan
            )
 
        self.scene.sensors = self._sensors # dict{uid: Camera} 
        self.scene.human_render_cameras = self._human_render_cameras # dict{uid: Camera}
    ...
    def render_human(self):
        ...
        for obj in self._hidden_objects:
            obj.show_visual() # obj | <class 'mani_skill.utils.structs.actor.Actor'>
        if self._viewer is None:
    ...
    def render_rgb_array(self, camera_name: str = None):
        ...
        # render_images['render_camera_left'] : torch.Size([1, 256, 256, 3])
        # 获取图像张量
        # image_tensor = render_images['render_camera_right']  # torch.Size([1, 256, 256, 3])

        # # 移除批次维度 (Batch Dimension)
        # image_tensor = image_tensor.squeeze(0)  # 变成 torch.Size([256, 256, 3])

        # # 确保数据在 [0, 1] 之间（如果需要）
        # if image_tensor.max() > 1:
        #     image_tensor = image_tensor / 255.0  # 归一化到 0-1 之间
